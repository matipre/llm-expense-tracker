...
class MessageProcessorService:
    ...
    async def process_message(self, message: IncomingMessage) -> None:
        ...
        try:
            ...
            if not is_expense:
                self.logger.info(f"Message from user {message.telegram_user_id} is not about an expense, ignoring")
                return
            ...
            if not parsed_expense:
                self.logger.warning(f"Failed to parse expense from message: {message.message_text}")
                await self._send_error_response(message, "I couldn't understand your expense. Please try again.")
                return
            ...
            await self._send_error_response(message, "An error occurred while processing your expense.")
