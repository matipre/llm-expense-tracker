...
class PostgreSQLUserRepository(IUserRepository):
    ...
    async def find_by_telegram_id(self, telegram_id: str) -> Optional[User]:
        ...
        try:
            ...
            self.logger.error(f"Error finding user by telegram_id {telegram_id}: {e}", exc_info=True)
            raise
    ...
    async def create(self, user: User) -> User:
        ...
        try:
            ...
            self.logger.error(f"Error creating user {user.telegram_id}: {e}", exc_info=True)
            raise
