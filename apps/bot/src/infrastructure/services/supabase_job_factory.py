...
class SupabaseJob(Job):
    ...
    async def schedule_task(self, data: Any) -> None:
        ...
        try:
            response = (
                self.supabase.schema("pgmq_public")
                .rpc(
                    "send", {"queue_name": self.options.name, "message": {"data": data}}
                )
                .execute()
            )
            ...
            if hasattr(response, "error") and response.error:
                self.logger.error(
                    f"Failed to send message to queue {self.options.name}: {response.error}"
                )
            ...
            self.logger.error(
                f"Error scheduling task for job '{self.options.name}': {e}"
            )
            raise
    ...
    async def _create_worker(self) -> None:
        ...
        self.logger.info(
            f"Starting worker for job '{self.options.name}'. QueueName: {self.options.name}"
        )
        ...
        while self._running:
            try:
                ...
                if hasattr(response, "error") and response.error:
                    self.logger.error(
                        f"Error reading from queue {self.options.name}: {response.error}"
                    )
                ...
                for msg in messages:
                    ...
                    self.logger.debug(
                        f"Processing job '{self.options.name}' with id {msg['msg_id']}"
                    )

                    try:
                        # Call handler with message data
                        result = await self.options.handler(
                            TaskHandlerArgs(data=(msg["message"]["data"]))
                        )

                        # Handle result based on status
                        if result.status == "error":
                            self.logger.error(
                                f"Job failed '{self.options.name}' with id {msg['msg_id']}: {result.result_message}"
                            )
                            # Archive failed messages
                            await self._archive_message(msg["msg_id"])
                        elif result.status == "cancelled":
                            self.logger.info(
                                f"Job cancelled '{self.options.name}' with id {msg['msg_id']}"
                            )
                            ...
                            await self._delete_message(msg["msg_id"])
                            self.logger.info(
                                f"Job completed '{self.options.name}' with id {msg['msg_id']}"
                            )

                    except Exception as e:
                        self.logger.error(
                            f"Error processing job '{self.options.name}' with id {msg['msg_id']}: {e}"
                        )
                        # Archive failed messages
                        await self._archive_message(msg["msg_id"])
    ...
    async def _delete_message(self, message_id: str) -> None:
        ...
        try:
            response = (
                ...
                .rpc(
                    "delete",
                    {"queue_name": self.options.name, "message_id": message_id},
                )
                .execute()
            )
            ...
            if hasattr(response, "error") and response.error:
                self.logger.error(
                    f"Error deleting message {message_id}: {response.error}"
                )
        except Exception as e:
    ...
    async def _archive_message(self, message_id: str) -> None:
        ...
        try:
            ...
            if hasattr(response, "error") and response.error:
                self.logger.error(
                    f"Error archiving message {message_id}: {response.error}"
                )
        except Exception as e:
