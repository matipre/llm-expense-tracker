...
from domain.entities.message import BotResponse, IncomingMessage
...
@pytest.mark.asyncio
async def test_has_supabase_queues_returns_false_when_pgmq_not_available(
    queue_service, mock_supabase_client
):
...
@pytest.mark.asyncio
async def test_has_supabase_queues_returns_true_when_pgmq_available(
    queue_service, mock_supabase_client
):
...
@pytest.mark.asyncio
async def test_send_response_uses_table_queue_when_pgmq_not_available(
    queue_service, mock_supabase_client
):
    """Test that sendResponse uses table-based queue when pgmq is not available."""
    response = BotResponse(
        chat_id=123456789, text="Test response", reply_to_message_id=456
    )
...
@pytest.mark.asyncio
async def test_send_response_uses_supabase_queue_when_pgmq_available(
    queue_service, mock_supabase_client
):
...
@pytest.mark.asyncio
async def test_consume_messages_handles_incoming_messages(
    queue_service, mock_supabase_client
):
